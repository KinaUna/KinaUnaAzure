...
namespace KinaUnaProgenyApi.Tests.Services.TodosServices
{
    public class TodosServiceTests
    {
        ...
        public TodosServiceTests()
        {
            ...
            // Initialize service
            _service = new TodosService(_progenyDbContext, _mockAccessManagementService.Object);
        ...
        public async Task GetTodoItem_WhenUserHasAccess_ReturnsTodoItemWithSubtaskCounts()
        {
            ...
            _mockAccessManagementService
                ...
                .ReturnsAsync(true);
            _mockAccessManagementService
                .Setup(x => x.GetItemPermissionForUser(KinaUnaTypes.TimeLineType.TodoItem, todoId, 1, 0, _testUser))
        ...
        public async Task GetTodosList_ReturnsNonDeletedTodosWithAccess()
        {
            ...
            // Act
            List<TodoItem> result = await _service.GetTodosList(progenyId, _testUser);

            // Assert
            Assert.NotNull(result);
            Assert.Equal(4, result.Count); // Including subtasks
            Assert.All(result, item => Assert.False(item.IsDeleted));
        }
