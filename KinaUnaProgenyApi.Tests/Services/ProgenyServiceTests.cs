    ...
    public class ProgenyServiceTests
    {
        ...
        public async Task AddProgeny_Should_Add_Progeny_To_Database()
        {
            ...
            mockUserGroupsService
                .Setup(x => x.AddUserGroup(It.IsAny<UserGroup>(), userInfo))
                .ReturnsAsync((UserGroup ug, UserInfo _) => { ug.UserGroupId = 1; return ug; });

            mockUserGroupsService
                .Setup(x => x.AddUserGroupMember(It.IsAny<UserGroupMember>(), userInfo))
                .ReturnsAsync((UserGroupMember ugm, UserInfo _) => { ugm.UserGroupMemberId = 1; return ugm; });

            mockKinaUnaCacheService
                .Setup(x => x.SetProgenyOrFamilyUpdatedCache(It.IsAny<int>(), It.IsAny<int>()))
        ...
        public async Task AddProgeny_Should_Grant_Admin_Permissions_To_Admins()
        {
            ...
            // Assert
            mockUserGroupsService.Verify(
                x => x.AddUserGroupMember(
                    It.Is<UserGroupMember>(ugm => ugm.Email == "admin@test.com"),
                    userInfo),
                Times.Once);
        }
        ...
        public async Task AddProgeny_Should_Grant_View_Permission_To_Progeny_User()
        {
            ...
            mockUserGroupsService.Verify(
                x => x.AddUserGroupMember(
                    It.Is<UserGroupMember>(ugm => ugm.Email == "child@test.com"),
                    userInfo),
                Times.Once);
        }
        ...
        public async Task UpdateProgeny_Should_Update_Admins_Permissions_When_Admins_Changed()
        {
            ...
            // Assert
            Assert.NotNull(result);
            mockAccessManagementService.Verify(x => x.ProgenyAdminsUpdated(1), Times.Once);
        }
        ...
        public async Task DeleteProgeny_Should_Delete_Picture_From_ImageStore()
        {
            ...
            // Assert
            Assert.NotNull(result);
            mockImageStore.Verify(x => x.DeleteImage("custom-picture.jpg", "progeny"), Times.Once);
        }
        ...
        public async Task DeleteProgenyInfo_Should_Delete_Associated_Address()
        {
            ...
            // Assert
            Assert.NotNull(result);
            mockLocationService.Verify(x => x.RemoveAddressItem(1), Times.Once);
        }
