...
namespace KinaUnaProgenyApi.Services.FamiliesServices
{
        ...
        IFamilyAuditLogsService familyAuditLogService, IProgenyService progenyService) : IFamilyMembersService
    {
        ...
        /// <returns></returns>
        public async Task UpdateFamilyMembersForNewUser(UserInfo userInfo)
        {
            List<FamilyMember> familyMembers = await progenyDbContext.FamilyMembersDb.Where(fm => fm.Email.ToUpper() == userInfo.UserEmail.ToUpper()).ToListAsync();
            if (familyMembers.Count == 0) return;
            
            foreach (FamilyMember familyMember in familyMembers)
            {
                familyMember.UserId = userInfo.UserId;
                progenyDbContext.FamilyMembersDb.Update(familyMember);
            }
