import { getFormattedDateString, getLongDateTimeFormatMoment, getZebraDateTimeFormat, setContextAutoSuggestList, setLocationAutoSuggestList, setMomentLocale, setTagsAutoSuggestList, TimelineChangedEvent, validateDateValue } from '../data-tools-v12.js';
import { addTimelineItemEventListener, showPopupAtLoad } from '../item-details/items-display-v12.js';
import * as pageModels from '../page-models-v12.js';
import { getSelectedFamilies, getSelectedProgenies } from '../settings-tools-v12.js';
import { startFullPageSpinner, startLoadingItemsSpinner, startTopMenuSpinner, stopFullPageSpinner, stopLoadingItemsSpinner, stopTopMenuSpinner } from '../navigation-tools-v12.js';
import * as SettingsHelper from '../settings-tools-v12.js';
import * as LocaleHelper from '../localization-v12.js';
import { popupTodoItem, setAssignStatusButtonsEventListeners } from './todo-details-v12.js';
...
function addTodoElementEventListeners(todoItemId) {
    ...
    const moreInfoButton = document.querySelector('[data-todo-item-more-info-button-id="' + todoItemId + '"]');
    if (moreInfoButton) {
    ...
    const todoElementsWithDataAssignToId = document.querySelectorAll('[data-todo-assign-to-id="' + todoItemId + '"]');
...
async function assignTodoItemTo(todoItemId) {
    ...
    if (response.ok) {
        ...
        if (modalDiv) {
            ...
            const addTodoItemForm = modalDiv.querySelector('#assign-todo-item-to-form');
            if (addTodoItemForm) {
...
async function addTodoItemToBoard(todoItemId) {
    ...
    if (response.ok) {
        ...
        if (modalDiv) {
            ...
            if (addTodoItemForm) {
                const addTodoItemFormFunction = async function (event) {
                    event.preventDefault();
                    startFullPageSpinner();
                    const formData = new FormData(addTodoItemForm);
...
const showMoreTodoInfo = function (event) {
    event.preventDefault();
    event.stopPropagation();
...
function hideTodoItemMoreInfoDivs(todoItemId) {
    ...
    const allMoreTodoItemButtons = document.querySelectorAll('.kanban-card-menu-button[data-todo-item-more-info-button-id]');
...
async function initialSettingsPanelSetup() {
    ...
    statusTypeButtons.forEach(function (button) {
        const toggleTodoFilterStatusTypeFunction = function (event) {
    ...
    const assignedToButtons = document.querySelectorAll('.filter-assigned-to-button');
    assignedToButtons.forEach(function (button) {
        const toggleTodoFilterAssignedToFunction = function (event) {
...
function dispatchTodoItemChangedEvent(todoItemId) {
...
//# sourceMappingURL=todos-index-v12.js.map