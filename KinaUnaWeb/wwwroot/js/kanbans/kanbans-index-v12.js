...
import { setContextAutoSuggestList, setTagsAutoSuggestList } from "../data-tools-v12.js";
...
import { KanbanBoardElementParameters, KanbanBoardsPageParameters, TimelineItem, TimeLineType } from "../page-models-v12.js";
...
const kanbansIndexPageParametersDiv = document.querySelector('#kanbans-index-page-parameters-div');
const kanbansListDiv = document.querySelector('#kanban-boards-list-div');
const todosPageMainDiv = document.querySelector('#kinauna-main-div');
let moreKanbanBoardsButton;
const itemsPerPageInput = document.querySelector('#kanbans-index-items-per-page-input');
/**
 * Sets the kanbans page parameters from the data attributes of the kanbansIndexPageParametersDiv.
 */
function setKabansPageParametersFromPageData() {
...
async function getKanbanBoards() {
    ...
    if (getMoreKanbanBoardsResponse.ok && getMoreKanbanBoardsResponse.body !== null) {
        const kanbanBoardsPageResponse = await getMoreKanbanBoardsResponse.json();
...
async function refreshKanbanBoards(changedKanbanBoardId) {
    ...
    if (parseInt(changedKanbanBoardId) > 0) {
        const kanbanBoardElement = document.querySelector(`[data-todo-id="${changedKanbanBoardId}"]`);
...
function increaseKanbanBoardsItemsPerPage() {
    if (itemsPerPageInput !== null) {
        let itemsPerPageInputValue = itemsPerPageInput.valueAsNumber;
        itemsPerPageInputValue++;
...
function toggleShowFilters() {
    const filtersElements = document.querySelectorAll('.kanbans-filter-options');
...
function loadKanbansPageSettings() {
    const pageSettingsFromStorage = SettingsHelper.getPageSettings(kanbansPageSettingsStorageKey);
...
async function initialSettingsPanelSetup() {
    const kanbansPageSaveSettingsButton = document.querySelector('#kanbans-page-save-settings-button');
    ...
    const toggleShowFiltersButton = document.querySelector('#kanbans-toggle-filters-button');
    if (toggleShowFiltersButton !== null) {
        const toggleShowFiltersFunction = function (event) {
...
function updateSettingsNotificationDiv() {
    let kanbansSettingsNotificationText = '';
 ...
 */
async function saveKanbansPageSettings() {
    const numberOfItemsToGetInput = document.querySelector('#kanbans-index-items-per-page-input');
    ...
    const setAsDefaultCheckbox = document.querySelector('#kanbans-settings-save-default-checkbox');
    if (setAsDefaultCheckbox !== null && setAsDefaultCheckbox.checked) {
        SettingsHelper.savePageSettings(kanbansPageSettingsStorageKey, kanbanBoardsPageParameters);
    }
...
//# sourceMappingURL=kanbans-index-v12.js.map