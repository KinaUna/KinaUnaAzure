...
import { dispatchKanbanBoardChangedEvent, getKanbanBoard, getKanbanItems, hideCardMenus, hideColumnMenus, setKanbanItems, updateKanbanItemsInColumn } from "./kanban-board-details-v12.js";
    ...
    if (kanbanItemId) {
        ...
        if (menuContentDiv) {
            if (menuContentDiv.classList.contains('d-none')) {
                ...
                let kanbanItems = getKanbanItems();
                let kanbanItem = kanbanItems.find(k => k.kanbanItemId.toString() === kanbanItemId);
                const showDetailsButton = menuContentDiv.querySelector('button[data-card-menu-action="showdetails"]');
                if (showDetailsButton) {
                    const showDetailsFunction = async function (event) {
                        event.preventDefault();
                        event.stopPropagation();
                ...
                const assignToButton = menuContentDiv.querySelector('button[data-card-menu-action="assignto"]');
                if (assignToButton) {
                    const assignToFunction = async function (event) {
                        event.preventDefault();
                        event.stopPropagation();
                        menuContentDiv.classList.add('d-none');
...
async function assignTo(kanbanItemId) {
    let kanbanItems = getKanbanItems();
    let kanbanItem = kanbanItems.find(k => k.kanbanItemId.toString() === kanbanItemId);
    if (kanbanItem) {
...
//# sourceMappingURL=kanban-cards-v12.js.map