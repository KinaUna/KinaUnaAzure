...
function addMonthDayNumberIconButtonEventListeners() {
    ...
    const yearlyDayNumberButtons = document.querySelectorAll('.yearly-month-day-number-icon');
...
function updateSelectedMonthDayNumbersInput() {
    ...
    if (frequencySelect === null || monthlyDayNumberInput === null)
        return;
    const frequencyValue = parseInt(frequencySelect.value);
    const monthDayNumbers = [];
    let monthDayNumberButtons = null;
    if (frequencyValue === 3) {
        monthDayNumberButtons = document.querySelectorAll('.month-day-number-icon');
    }
    if (frequencyValue === 4) {
        monthDayNumberButtons = document.querySelectorAll('.yearly-month-day-number-icon');
    }
    if (monthDayNumberButtons === null)
        return;
    monthDayNumberButtons.forEach((monthDayNumberButton) => {
...
function updateRecurrenceByDayInput() {
    const byDayInput = document.querySelector('#event-repeat-days-input');
    const frequencySelect = document.querySelector('#event-repeat-frequency-select');
...
function initializeRepeatMonthlyTypeRadioButtons() {
    ...
    if (byMonthDayInput !== null) {
        ...
        const repeatByMonthSelectedType = document.querySelector('#event-repeat-monthly-type-selected-input');
        if (dayNumberTypeRadioButton !== null && dayPatternTypeRadioButton !== null) {
            if (repeatByMonthSelectedType?.value === '0') {
...
function initializeRepeatMonthDayNumberInput() {
    ...
    if (monthlyDayNumberInput !== null) {
        ...
        monthlyDayNumbers.forEach((dayNumber) => {
            const monthlyDayButton = document.querySelector('.month-day-number-icon[data-monthday="' + dayNumber + '"]');
            if (monthlyDayButton !== null) {
                monthlyDayButton.classList.add('selected');
            }
            const yearlyDayButton = document.querySelector('.yearly-month-day-number-icon[data-monthday="' + dayNumber + '"]');
...
function initializeRepeatMonthDaysInput() {
    ...
    const selectedMonthlyDaysPrefixes = document.querySelectorAll('.event-repeat-monthly-by-day-prefix-checkbox');
    ...
    if (eventRepeatMonthlyDaysInput !== null && selectedMonthlyDaysPrefixes !== null) {
        ...
        selectedDays.forEach((day) => {
            ...
            const dayPrefixCheckbox = document.querySelector('.event-repeat-monthly-by-day-prefix-checkbox[value="' + dayStartString + '"]');
...
function addMonthlyAndYearlyByDayPrefixEventListeners() {
    const monthlyByDayPrefixCheckboxes = document.querySelectorAll('.event-repeat-monthly-by-day-prefix-checkbox');
    ...
    const yearlyByDayPrefixCheckboxes = document.querySelectorAll('.event-repeat-yearly-by-day-prefix-checkbox');
...
function updateRepeatMonthlyDaysInput() {
    ...
    if (frequencySelect === null)
        return;
    let eventRepeatMonthlyDaysInput = document.querySelector('#event-repeat-monthly-days-input');
    const frequencyValue = parseInt(frequencySelect.value);
    ...
    if (frequencyValue === 4) {
        eventRepeatMonthlyDaysInput = document.querySelector('#event-repeat-yearly-days-input');
        const yearlyByDayPrefixCheckboxes = document.querySelectorAll('.event-repeat-yearly-by-day-prefix-checkbox');
        yearlyByDayPrefixCheckboxes.forEach((yearlyByDayPrefixCheckbox) => {
            if (yearlyByDayPrefixCheckbox.checked) {
                selectedPrefixes.push(yearlyByDayPrefixCheckbox.value);
            }
        });
        const yearlyMonthDayButtons = document.querySelectorAll('.yearly-monthday-icon');
        yearlyMonthDayButtons.forEach((yearlyMonthDayButton) => {
            if (yearlyMonthDayButton.classList.contains('selected')) {
                if (yearlyMonthDayButton.dataset.weekday) {
                    selectedMonthDays.push(yearlyMonthDayButton.dataset.weekday);
                }
            }
        });
    }
...
function initializeRepeatYearDaysInput() {
    const yearlyWeekDaysButtons = document.querySelectorAll('.yearly-monthday-icon');
    yearlyWeekDaysButtons.forEach((buttonElement) => {
        buttonElement.classList.remove('selected');
    });
    const selectedYearlyDaysPrefixes = document.querySelectorAll('.event-repeat-yearly-by-day-prefix-checkbox');
    ...
    if (eventRepeatYearlyDaysInput !== null && selectedYearlyDaysPrefixes !== null) {
        ...
        selectedDays.forEach((day) => {
            // Get the input with the dayStartString value
            let dayStartString = day.slice(0, -2);
            const dayPrefixCheckbox = document.querySelector('.event-repeat-yearly-by-day-prefix-checkbox[value="' + dayStartString + '"]');
            if (dayPrefixCheckbox !== null) {
                dayPrefixCheckbox.checked = true;
            }
            // Get the button with the data-weekday attribute that matches the day
            let dayEndString = day.slice(-2);
            const dayButton = document.querySelector('.yearly-monthday-icon[data-weekday="' + dayEndString + '"]');
            if (dayButton !== null) {
                dayButton.classList.add('selected');
            }
        });
...
function addYearlyMonthDayIconButtonEventListeners() {
    const monthDayButtons = document.querySelectorAll('.yearly-monthday-icon');
    monthDayButtons.forEach((monthDayButton) => {
...
function onYearlyMonthDayButtonClicked(evt) {
    evt.preventDefault();
    const monthDayButtonElement = evt.currentTarget;
    if (monthDayButtonElement !== null) {
        if (monthDayButtonElement.classList.contains('selected')) {
...
function initializeRepeatYearlyTypeRadioButtons() {
    ...
    if (byMonthDayInput !== null) {
        ...
        if (dayNumberTypeRadioButton !== null && dayPatternTypeRadioButton !== null) {
            if (repeatByYearSelectedType?.value === '0') {
                dayNumberTypeRadioButton.checked = true;
                dayPatternTypeRadioButton.checked = false;
                repeatByMonthDayNumberDiv?.classList.remove('d-none');
                repeatByMonthDaysPatternDiv?.classList.add('d-none');
...
export function setupRecurrenceSection() {
    ...
    initializeRepeatMonthDaysInput();
    initializeRepeatMonthDayNumberInput();
    addMonthlyAndYearlyByDayPrefixEventListeners();
    ...
    initializeRepeatYearDaysInput();
    initializeRepeatYearlyTypeRadioButtons();
    addYearlyMonthDayIconButtonEventListeners();
    addEventYearlyTypeRadioButtonsEventListeners();
}
//# sourceMappingURL=add-edit-recurrence-v11.js.map