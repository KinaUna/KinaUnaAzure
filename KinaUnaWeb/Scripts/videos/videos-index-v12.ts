...
import { Video, VideoViewModel, VideosList, VideosPageParameters, TimelineItem, TimeLineType } from '../page-models-v12.js';
import { getCurrentProgenyId, getCurrentLanguageId, setMomentLocale, getZebraDateTimeFormat, getLongDateTimeFormatMoment, getFormattedDateString } from '../data-tools-v12.js';
import * as SettingsHelper from '../settings-tools-v12.js';
import { startLoadingItemsSpinner, startTopMenuSpinner, stopLoadingItemsSpinner, stopTopMenuSpinner } from '../navigation-tools-v12.js';
import { addTimelineItemEventListener, showPopupAtLoad } from '../item-details/items-display-v12.js';
...
let languageId = 1;
let videosPageProgenyId: number;
...
const videosListDiv = document.querySelector<HTMLDivElement>('#video-list-div');
 ...
 */
function getPageParametersFromPageData(): VideosPageParameters | null {
    ...
    if (videosPageParametersDiv !== null) {
        ...
        if (pageParametersString) {
            let videosPageParametersFromPageData: VideosPageParameters = JSON.parse(pageParametersString);
 ...
 */
async function getVideosList(parameters: VideosPageParameters, updateHistory: Boolean = true): Promise<VideosPageParameters> {
    runLoadingSpinner();
    ...
    await fetch('/Videos/GetVideoList', {
        ...
        if (getItemsListResult != null) {
            const newItemsList = (await getItemsListResult.json()) as VideosList;
 ...
 */
async function processVideosList(newItemsList: VideosList, parameters: VideosPageParameters): Promise<VideosPageParameters> {
    ...
    const videoListParentDiv = document.querySelector<HTMLDivElement>('#video-list-parent-div');
 ...
 */
function setBrowserUrl(parameters: VideosPageParameters, replaceState: boolean): void {
...
async function loadPageFromHistory(): Promise<void> {
    ...
    if (videosPageParameters !== null) {
        ...
        videosPageParameters.progenyId = url.searchParams.get('childId') ? parseInt(url.searchParams.get('childId') as string) : 0;
        videosPageParameters.sort = url.searchParams.get('sortBy') ? parseInt(url.searchParams.get('sortBy') as string) : 1;
        videosPageParameters.itemsPerPage = url.searchParams.get('pageSize') ? parseInt(url.searchParams.get('pageSize') as string) : 10;
        videosPageParameters.tagFilter = url.searchParams.get('tagFilter') ? url.searchParams.get('tagFilter') as string : '';
        videosPageParameters.year = url.searchParams.get('year') ? parseInt(url.searchParams.get('year') as string) : 0;
        videosPageParameters.month = url.searchParams.get('month') ? parseInt(url.searchParams.get('month') as string) : 0;
        videosPageParameters.day = url.searchParams.get('day') ? parseInt(url.searchParams.get('day') as string) : 0;
        videosPageParameters.sortTags = url.searchParams.get('sortTags') ? parseInt(url.searchParams.get('sortTags') as string) : 0;
...
*/
function updateActiveTagFilterDiv(parameters: VideosPageParameters): void {
...
*/
async function renderVideoItem(videoItem: Video): Promise<void> {
    if (videosPageParameters !== null) {
...
function clearVideoElements(): void {
    ...
    const videoListDiv = document.querySelector<HTMLDivElement>('#video-list-div');
 ...
 */
function sortVideosAscending(): void {
    if (videosPageParameters === null) {
        return;
    }
 ...
 */
function sortVideosDescending(): void {
    if (videosPageParameters === null) {
        return;
    }
 ...
 */
async function saveVideosPageSettings(): Promise<void> {
    if (videosPageParameters === null) {
        return new Promise<void>(function (resolve, reject) {
            reject();
        });
    ...
    if (saveAsDefaultCheckbox !== null && saveAsDefaultCheckbox.checked) {
        SettingsHelper.savePageSettings<VideosPageParameters>(videosPageSettingsStorageKey, videosPageParameters);
    }
 ...
 */
function loadVideosPageParametersFromStorage(): VideosPageParameters | null {
    let pageParametersFromStorage = new VideosPageParameters();

    let pageSettingsFromStorage: VideosPageParameters | null = SettingsHelper.getPageSettings<VideosPageParameters>(videosPageSettingsStorageKey);
    if (pageSettingsFromStorage !== null) {
 ...
 * @param parameters
 */
function updateNavigationDiv(parameters: VideosPageParameters): void {
