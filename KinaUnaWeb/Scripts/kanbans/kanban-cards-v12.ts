...
import { KanbanItem } from "../page-models-v12.js";
import { getStatusIconForTodoItems } from "../todos/todo-details-v12.js";
import { dispatchKanbanBoardChangedEvent, getKanbanBoard, getKanbanItems, hideAllMenusAndModals, hideCardMenus, hideColumnMenus, setKanbanItems, updateKanbanItemsInColumn } from "./kanban-board-details-v12.js";
import { displayKanbanItemDetails, getRemoveKanbanItemForm } from "./kanban-items-v12.js";
...
export function createKanbanItemCardHTML(kanbanItem: KanbanItem):HTMLDivElement {
...
export function addCardEventListeners(kanbanItemId: number, userCanEdit: boolean) {
    ...
    const cardClickFunction = async function (event: MouseEvent) {
...
export function addCardDividerEventListeners(columnId: number, rowIndex: number) {
    const dividerElements = document.querySelectorAll<HTMLDivElement>('.kanban-card-divider');
    dividerElements.forEach((divider) => {
        ...
        if (divider.dataset.columnId === columnId.toString() && divider.dataset.rowIndex === rowIndex.toString()) {
            const dividerDragEnterFunction = function (event: DragEvent) {
                event.preventDefault();
                event.stopPropagation();
                ...
                let kanbanItemId = '';
                if (draggedCard) {
                    const draggedCardDiv = draggedCard as HTMLDivElement;
                    if (draggedCardDiv) {
            ...
            const dividerDragOverFunction = function (event: DragEvent) {
                event.preventDefault();
                event.stopPropagation();
                // Check if event.dataTransfer contains kanban-item-card
                if (!event.dataTransfer || !event.dataTransfer.types.includes('kanban-item-card')) {
            ...
            const dividerDragLeaveFunction = function (event: DragEvent) {
                event.preventDefault();
                event.stopPropagation();
            ...
            const dividerDropFunction = async function (event: DragEvent) {
                event.preventDefault();
                event.stopPropagation();
                divider.classList.remove('kanban-card-divider-drag-over');
                if (event.dataTransfer) {
    ...
    if (kanbanItemId) {
        ...
        if (menuContentDiv) {
            if (menuContentDiv.classList.contains('d-none')) {
                ...
                const copyCardToButton = menuContentDiv.querySelector<HTMLButtonElement>('button[data-card-menu-action="copytoboard"]');
                if (copyCardToButton) {
                    const copyCardToFunction = async function (event: MouseEvent) {
                        event.preventDefault();
                        event.stopPropagation();
                        menuContentDiv.classList.add('d-none');
                ...
                const moveCardToButton = menuContentDiv.querySelector<HTMLButtonElement>('button[data-card-menu-action="movetoboard"]');
                if (moveCardToButton) {
                    const moveCardToFunction = async function (event: MouseEvent) {
                        event.preventDefault();
                        event.stopPropagation();
                        menuContentDiv.classList.add('d-none');
...
async function copyCardToBoard(kanbanItemId: string) {
    ...
    if (response.ok) {
        ...
        if (modalDiv) {
            ...
            const copyKanbanItemForm = modalDiv.querySelector<HTMLFormElement>('#copy-kanban-item-to-kanban-board-form');
...
async function moveCardToBoard(kanbanItemId: string) {
    // Get form html from server.
    ...
    if (response.ok) {
        ...
        if (modalDiv) {
            modalDiv.innerHTML = formHtml;
            modalDiv.classList.remove('d-none');
            const cancelButton = modalDiv.querySelector<HTMLButtonElement>('.move-kanban-item-cancel-button');
            if (cancelButton) {
                const closeButtonFunction = function () {
                    modalDiv.innerHTML = '';
                    modalDiv.classList.add('d-none');
            ...
            const moveKanbanItemForm = modalDiv.querySelector<HTMLFormElement>('#move-kanban-item-to-kanban-board-form');
