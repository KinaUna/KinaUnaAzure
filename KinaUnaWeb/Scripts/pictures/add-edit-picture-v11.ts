...
import { setTagsAutoSuggestList, setLocationAutoSuggestList, getCurrentProgenyId, getCurrentLanguageId, setMomentLocale, getZebraDateTimeFormat } from '../data-tools-v11.js';
import { startLoadingItemsSpinner, stopLoadingItemsSpinner } from '../navigation-tools-v11.js';
import { Picture, PictureViewModel, TimelineItem, TimeLineType } from '../page-models-v11.js';
import { addCopyLocationButtonEventListener } from '../locations/location-tools-v11.js';
import { setAddItemButtonEventListeners } from '../addItem/add-item-v11.js';
import { renderItemPermissionsEditor, setPermissions } from '../item-permissions-v11.js';
...
function hideInputsWhenUploading(): void {
    const selectphotosButtonParentDiv = document.getElementById('select-photos-button-parent-div') as HTMLDivElement;
    if (selectphotosButtonParentDiv !== null) {
        selectphotosButtonParentDiv.classList.add('d-none');
    }

    const saveButton = document.getElementById('add-picture-save-button') as HTMLButtonElement;
    if (saveButton !== null) {
        saveButton.disabled = true;
        saveButton.classList.add('d-none');
    }
    const selectPhotosButton = document.getElementById('select-photos-button') as HTMLButtonElement;
    if (selectPhotosButton !== null) {
        selectPhotosButton.disabled = true;
        selectPhotosButton.classList.add('d-none');
    }

    const suggestTagsInputs = document.querySelectorAll<HTMLInputElement>('.amsify-suggestags-input');
    suggestTagsInputs.forEach((input) => {
        input.disabled = true;

    });

    const accessLevelButtons = document.querySelectorAll<HTMLInputElement>('.btn-kinaunaselect');
    accessLevelButtons.forEach((button) => {
        button.disabled = true;
    });

    const locationFormGroup = document.getElementById('location-form-group') as HTMLDivElement;
    if (locationFormGroup !== null) {
        locationFormGroup.classList.add('d-none');
    }

    const tagsFormGroup = document.getElementById('tags-form-group') as HTMLDivElement;
    if (tagsFormGroup !== null) {
        tagsFormGroup.classList.add('d-none');
    }

    const accessLevelFormGroup = document.getElementById('access-level-form-group') as HTMLDivElement;
    if (accessLevelFormGroup !== null) {
        accessLevelFormGroup.classList.add('d-none');
    }

    const actionsFormGroup = document.getElementById('actions-form-group') as HTMLDivElement;
    ...
    if (notSupportedFiles.length > 0) {
        for (let fileItem of notSupportedFiles) {
            const picturePreviewDiv = document.getElementById('picture-preview-div' + notSupportedFiles.indexOf(fileItem)) as HTMLDivElement;
    ...
    const photoHeaderLabel = document.getElementById('photo-header-label') as HTMLDivElement;
 ...
 */
function showLoadingSpinners(): void {
    let itemNumber = 1;
...
async function uploadPicture(formData: FormData, pictureFile: File, itemNumber: number): Promise<void> {
    ...
    if (response) {
        const pictureItem = (await response.json()) as Picture;
...
async function selectFiles(): Promise<void> {
    ...
    const uploadFileDiv = document.getElementById('upload-file-div') as HTMLDivElement;
    uploadFileDiv.innerHTML = '';
    const fileHandles = await (window as any).showOpenFilePicker(filePickerOptions);
 ...
 */
async function waitForDelay(milliseconds: number): Promise<any> {
 ...
 */
function addDropEventListener(): void {
    const dropZone = document.getElementById('drop-files-div');
    if (dropZone === null) {
        return;
    }
 ...
 */
function addFileInputEventListener(): void {
    const filesInput = document.querySelector<HTMLInputElement>('#select-photos-button');
    if (filesInput === null) {
        return;
    }
 ...
 */
async function displayFile(file: File, fileType: string): Promise<void> {
    ...
    removeFileButton.addEventListener('click', async (event) => {
        ...
        const picturePreviewDivToRemove = document.getElementById(removeFilePicturePreviewDivId) as HTMLDivElement;
 ...
 */
async function displayNotSupportedFile(file: File): Promise<void> {
    const picturePreviewDiv = document.createElement('div');
