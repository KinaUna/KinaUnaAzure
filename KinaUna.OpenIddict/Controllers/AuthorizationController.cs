...
using KinaUna.OpenIddict.Models.Authorization;
...
public class AuthorizationController : Controller
{
    ...
    {
        // Debugging - add this line to log authentication status
        Console.WriteLine($"User authenticated: {User.Identity?.IsAuthenticated}, Authentication type: {User.Identity?.AuthenticationType}");

        // Rest of method...
        // When authenticating, make sure to use the correct authentication scheme:
        AuthenticateResult authResult = await HttpContext.AuthenticateAsync(IdentityConstants.ApplicationScheme);

        // Log the result
        Console.WriteLine($"Authentication result succeeded: {authResult.Succeeded}");
        ...
        if (request.HasPromptValue(PromptValues.Login))
        {
            ...
            List<KeyValuePair<string, StringValues>> parameters = Request.HasFormContentType ? Request.Form.Where(parameter => parameter.Key != Parameters.Prompt).ToList() : Request.Query.Where(parameter => parameter.Key != Parameters.Prompt).ToList();
    ...
    public async Task<IActionResult> Exchange()
    {
        ...
        if (request.IsClientCredentialsGrantType())
        {
            ...
            // Set the audiences.
            // Note: the audiences are used by OpenIddict to determine which access tokens
            // should be considered valid by the APIs that are protected by OpenIddict.
            identity.AddClaim(Claims.Audience, await _applicationManager.GetClientIdAsync(application));
        ...
        if (principal == null)
            return Forbid(
                ...
                properties: new AuthenticationProperties(new Dictionary<string, string?>
                {
                    ...
                    [OpenIddictServerAspNetCoreConstants.Properties.ErrorDescription] = "The principal is not valid."
                }));
    ...
    private IEnumerable<string> GetDestinations(Claim claim, ClaimsPrincipal principal)
    {
        ...
        {
            case Claims.Subject: 
                // Always include the subject in the access token, as it's used by the APIs to identify the user.
                yield return Destinations.AccessToken;
                // Always include the subject in the identity token, as it's used by the client application to identify the user.
            ...
            case Claims.Audience:
                // Always include the audience in the access token, as it's used by the APIs to validate the token.
                yield return Destinations.AccessToken;
