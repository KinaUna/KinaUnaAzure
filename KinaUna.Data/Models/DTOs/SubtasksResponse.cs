using System.Collections.Generic;

namespace KinaUna.Data.Models.DTOs
{
    /// <summary>
    /// Defines the response model for retrieving subtasks associated with a parent to-do item.
    /// </summary>
    public class SubtasksResponse
    {
        /// <summary>
        /// Gets or sets the identifier of the parent to-do item.
        /// </summary>
        public int ParentTodoItemId { get; set; } = 0;
        /// <summary>
        /// Gets or sets the collection of to-do items.
        /// </summary>
        public List<TodoItem> Subtasks { get; set; }
        
        /// <summary>
        /// Gets or sets the request object containing parameters used for retrieving the list of subtask items.
        /// </summary>
        public SubtasksRequest SubtasksRequest { get; set; }

        /// <summary>
        /// Gets or sets the current page number in a paginated collection.
        /// </summary>
        public int PageNumber { get; set; } = 0;

        /// <summary>
        /// Gets or sets the total number of pages available.
        /// </summary>
        public int TotalPages { get; set; } = 0;

        /// <summary>
        /// Gets or sets the total number of items.
        /// </summary>
        public int TotalItems { get; set; } = 0;

    }
}
